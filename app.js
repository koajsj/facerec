...
const cfg = { model: "detector", smoothFactor: 0.22, minConfidence: 0.6, showKeypoints: true, ttlMs: 300, detectEveryNFrames: 2 };
...
function resizeCanvas() {
  ...
  if (els.overlay.width !== w || els.overlay.height !== h) { els.overlay.width = w; els.overlay.height = h; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); }
}

function quality(face) {
  if (!face) return "未检测到人脸";
  const area = (face.w * face.h) / ((els.video.videoWidth || 1) * (els.video.videoHeight || 1));
  if (area < 0.06) return "距离偏远";
  if (area > 0.55) return "距离过近";
  return "稳定";
}
...
function loop(ts) {
  ...
  if (app.frame % cfg.detectEveryNFrames === 0) {
    const out = detector.detect(els.video, ts, { smoothFactor: cfg.smoothFactor, ttlMs: cfg.ttlMs, mode: "single", iouThreshold: 0.2 });
    app.faces = out.faces;
  ...
  els.metricsText.textContent = app.faces.length
    ? `置信度 ${(app.faces[0].scoreAvg * 100).toFixed(0)}% · 延迟 ${detector.avgLatencyMs.toFixed(1)}ms`
    : "未检测到人脸";
...
async function start() {
  ...
  await startCamera();
  app.running = true; app.frame = 0; app.lastVideoTime = -1;
...
function snapshot() {
  ...
  c.toBlob((b) => {
    ...
    a.href = URL.createObjectURL(b); a.download = `截图-${Date.now()}.png`; a.click();
