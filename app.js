...
const app = { running: false, stream: null, raf: 0, frame: 0, lastVideoTime: -1, faces: [], lockId: null, debugOn: false, fps: 0, fpsCount: 0, fpsTick: performance.now() };
const cfg = { mode: "single", smoothFactor: 0.22, minConfidence: 0.6, showKeypoints: true, privacyMode: true, ttlMs: 300, detectEveryNFrames: 2, profile: "balance" };
...
function render(faces) {
  ...
  faces.forEach((f, i) => {
    ...
    if (cfg.showKeypoints && f.keypoints?.length) { ctx.fillStyle = color; f.keypoints.forEach((p) => { ctx.beginPath(); ctx.arc(p.x * sx, p.y * sy, 2, 0, Math.PI * 2); ctx.fill(); }); }
  });
}
...
function updateDebug() {
  ...
  if (now - app.fpsTick >= 1000) {
    app.fps = app.fpsCount;
    app.fpsCount = 0;
    app.fpsTick = now;
  }
...
function loop(ts) {
  ...
  if (app.frame % cfg.detectEveryNFrames === 0) {
    app.faces = detector.detect(els.video, ts, {
...
async function start() {
  ...
  try {
    ...
    await startCamera();
    app.running = true; app.frame = 0; app.lastVideoTime = -1; app.lockId = null;
...
function stop() { if (!app.running) return; app.running = false; cancelAnimationFrame(app.raf); stopCamera(); detector.reset(); ctx.clearRect(0, 0, els.overlay.clientWidth, els.overlay.clientHeight); els.stopBtn.disabled = true; setState("stopped"); }

function snapshot() { if (!els.video.videoWidth) return; const c = document.createElement("canvas"); c.width = els.video.videoWidth; c.height = els.video.videoHeight; const cctx = c.getContext("2d"); cctx.save(); cctx.translate(c.width, 0); cctx.scale(-1, 1); cctx.drawImage(els.video, 0, 0, c.width, c.height); cctx.restore(); cctx.drawImage(els.overlay, 0, 0, c.width, c.height); c.toBlob((b) => { if (!b) return; const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = `face-shot-${Date.now()}.png`; a.click(); URL.revokeObjectURL(a.href); }); }
function exportJson() { const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), profile: cfg.profile }, null, 2)], { type: "application/json" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "face-tracker-session.json"; a.click(); URL.revokeObjectURL(a.href); }

function bind() {
  els.startBtn.onclick = start; els.stopBtn.onclick = stop; els.shotBtn.onclick = snapshot; els.exportBtn.onclick = exportJson;
  ...
  els.presetMeeting.onclick = () => { cfg.mode = "single"; cfg.smoothFactor = 0.28; cfg.minConfidence = 0.65; setPerfProfile("balance"); applyCfg(); saveCfg(); };
  els.presetSelfie.onclick = () => { cfg.mode = "single"; cfg.smoothFactor = 0.18; cfg.minConfidence = 0.55; setPerfProfile("high"); applyCfg(); saveCfg(); };
  els.presetLow.onclick = () => { cfg.mode = "single"; cfg.smoothFactor = 0.35; cfg.minConfidence = 0.62; setPerfProfile("save"); applyCfg(); saveCfg(); };
